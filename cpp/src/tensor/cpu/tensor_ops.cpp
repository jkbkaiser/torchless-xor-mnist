...
Shape broadcast_shape(const Shape &shape1, const Shape &shape2) {
...
std::vector<std::vector<size_t>> all_indices(const Shape &shape) {
...
std::vector<size_t> broadcast_idx(std::vector<size_t> idx, const Shape &shape) {
...
CPUTensor CPUTensor::operator+(const CPUTensor &other) const {
    auto out_shape = broadcast_shape(this->shape_, other.shape_);
    std::cout << "Computed shape " << out_shape.dims_ << std::endl;

    CPUTensor out(out_shape);

    for (auto idx : all_indices(out_shape)) {
        auto idx1 = broadcast_idx(idx, this->shape_);
        auto idx2 = broadcast_idx(idx, other.shape_);

        out.set(idx, this->get(idx1) + other.get(idx2));
    }
